...
public class OptionGreeksSnapshotChartQuoteServlet extends HttpServlet
{	
		...
		throws ServletException,IOException
	{
		try
		{
	        ...
	        String forDateTime = request.getParameter("forDateTime");
	        boolean excludeOddStrikes = Boolean.parseBoolean(request.getParameter("excludeOddStrikes"));
	        float startingDelta = Float.parseFloat(request.getParameter("startingDelta"));
	        // String csvFilename = dataManager.getOptionDeltaRangeRawDataAnalysis(mainInstrumentId, forDate, baseDelta);	
	        byte[] csvContents = dataManager.getOptionGreeksSnapshot(instrument, forDateTime, excludeOddStrikes, startingDelta);
