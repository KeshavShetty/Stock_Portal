...
public class Daemon extends Thread
{
	...
	{
		int daemonPort;
		String fatherProxyHost=null;
		int fatherProxyPort = 0; 
		...
		switch (args.length)
		{
			...
			case 3: try
					{
						fatherProxyHost = args[1];
						daemonPort = Integer.parseInt(args[0]);
						fatherProxyPort = Integer.parseInt(args[2]);
					}
					catch (NumberFormatException e)
					{
						System.out.println("Error: Invalid daemon port");
			...
			default:System.out.println("Usage: Proxy [daemon port] [father proxy host] [father proxy port]");
					return;
		}
		...
		try
		{
			...
			if (fatherProxyHost!=null) {
				config.setFatherProxyHost(fatherProxyHost);
				config.setFatherProxyPort(fatherProxyPort);
			} 
