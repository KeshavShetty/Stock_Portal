...
public class Proxy extends Thread
{
	...
	String localHostName = "localhost";             // Local machine name
	String localHostIP = "127.0.0.1";               // Local machine IP address
	String adminPath = null;                 // Path of admin applet
	Config config = null;                    // Config object
	DateFormat DATE_FORMAT_WITHTIME = new SimpleDateFormat("dd-MM-yy HH:mm:ss");
    ...
    public void run()
	{
		...
		try
		{
            ...
            if (url.getPath().endsWith(".js")||url.getPath().endsWith(".jpg")||url.getPath().endsWith(".gif")||url.getPath().endsWith(".css")||url.getPath().endsWith(".png")) {
            	
            } else {
            	System.out.println(DATE_FORMAT_WITHTIME.format(new Date())+ " >> " + "Request = " + url);  
            }
			...
			{ 
				...
				//
				//System.out.println("Father"+config.getFatherProxyHost()); 
				if (config.getFatherProxyHost()!=null && config.getIsFatherProxy())  
				{
				...
				{
					//System.out.println("NOT Caching the reply. Reason:" +reasonForNotCaching);
				}                
				// 
